...
public class HomeAlertAdapter extends RecyclerView.Adapter<HomeAlertAdapter.myViewHodler>{
    private ArrayList<Home_item> Home_item_List;

    //建立建構函式
    public HomeAlertAdapter(ArrayList<Home_item> Home_item_List) {
        //改資料內容
     ...
     */
    @Override
    public HomeAlertAdapter.myViewHodler onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        //建立自定義佈局
        View itemView = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.home_article_card, viewGroup, false);
        return new HomeAlertAdapter.myViewHodler(itemView);
    }
     ...
     */
    @Override
    public void onBindViewHolder(HomeAlertAdapter.myViewHodler holder, int position) {
        //根據點選位置繫結資料
        //獲取實體類中的欄位並設定
    ...
    private HomeAlertAdapter.OnItemClickListener onItemClickListener;

    public void setOnItemClickListener(HomeAlertAdapter.OnItemClickListener onItemClickListener) {
